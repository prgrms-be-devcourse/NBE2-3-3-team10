...
class LaundryShopService {
    ...
    fun getLaundryShops(userLat: Double, userLng: Double): MutableList<LaundryShop?> {
        val shops = laundryShopRepository!!.findAll()
        return sortByDistance(shops, userLat, userLng)
    }

    // 검색된 세탁소 리스트 거리순 정렬
    fun getLaundryShops(shop_name: String, userLat: Double, userLng: Double): MutableList<LaundryShop?> {
        val shops = laundryShopRepository!!.findByShopNameContaining(shop_name)
        return sortByDistance(shops, userLat, userLng)
    }

    // 거리 계산 및 정렬
    private fun sortByDistance(shops: MutableList<LaundryShop?>, userLat: Double, userLng: Double): MutableList<LaundryShop?> {
        return shops.stream()
    ...
    fun setHandledItems(items: List<HandledItemsResDTO>): List<HandledItems> {
        ...
        // 전체 항목 저장
        return handledItemsRepository.saveAll<HandledItems>(
            handledItemsList
        )
    }
}
